singleton set vgdp(var) ;
singleton set vgdppc(var) ;

$gdxIn "%BASENAME%Scn.gdx"
$load vgdp
$load vgdppc
...
*display scen, r, tranche, mod, ssp, var, tt ;

Execute_loaddc "%BASENAME%Scn.gdx", popScen, popHist, gdpScen, PPP0=PPP, EXRREF, educScen, ghgHist ;

PPP(r) = PPP0(r,t0) ;

*  All incoming data is assumed to be in millions

popScen(scen, r, tranche, tt)      = popScen(scen, r, tranche, tt) ;
popHist(r,tranche,tt)              = popHist(r,tranche,tt) ;
educScen(scen, r, tranche, ed, tt) = educScen(scen, r, tranche, ed, tt) ;

if(%OVERLAYPOP%,
   popg(r) = popScen("%POPSCEN%", r, "PTOTL", t0) ;
else
...
pop.l(r,t) = popT(r, "PTOTL", t) ; pop0(r) = pop.l(r,t0) ;

*  Calculate GDP trend, assuming per capita growth rates are as given by scenario

rgdppcT(r,t) = gdpScen("%SSPMOD%", "%SSPSCEN%", vgdppc, r, t) ;
...
* execute_unload "xxx.gdx", ed, educmap, educScen, lfpr ; Abort$1 "Temp" ;
